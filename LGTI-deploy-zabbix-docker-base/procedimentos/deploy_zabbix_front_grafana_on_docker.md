...
```bash
docker node update --availability drain srvzabbix
```
...
sh grafana.sh
docker stack deploy -c docker-compose.yaml tecbassis
```
...
```bash
docker service logs -f tecbassis_zabbix-server
docker service logs -f tecbassis_zabbix-frontend
docker service logs -f tecbassis_grafana
```
 
### Removendo stack

```bash
docker stack rm tecbassis
```
